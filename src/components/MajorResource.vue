  ...
  <div>
    ...
    <div class="major-head">
      <div class="caree">
        <div class="sort">
         ...
         <div class="careerg">
            ...
            <div class="catalog">专业目录类别：××××专业大类>>××××专业小类</div>
            <div class="catalog">专业所属分类：××××专业/独立专业）</div>
         </div>
         </div>
         <div class="star">
      ...
      <div class="major-state">
        ...
        <el-cascader ref="cascaderAddr" :options="optionsList" :props="props" filterable :placeholder="placeholderValue" :show-all-levels="false" separator="-" style="width: 240px;" @change="handleChange" @focus="handleFocus">
        </el-cascader>
      ...
      <div class="major-state">
        ...
        <div class="item" :class="{ on: currentIndex === index }" v-for="(item, index) in majorStateList" :key="index" @Click="majorStateClick(item,index)">{{ item.name}}
        </div>
      </div>
    </div>
    <div class="major-outer">
      <div class="major-main">
        <div class="subitem" v-for="(item, index) in majorList" :key="index">
          <div class="lf">
          ...
          <div class="rg">
            ...
            <!-- <div class="content" ref="detailDom">
              ...
              <el-popover v-if="item.isMore" placement="right" :width="420" trigger="click" :title="item.name">
...
export default {
  ...
  mounted() {
    ...
    //console.log('aaaa', _this.majorId)
    ...
    _this.getMajorResourceList();

      // for (let i = 0; i < this.group.length; i++) { 
      //               this.group[i].checked= false   // 在数组里的对象中添加新的属性"num"
      //     }
  
  this.group.forEach(item => {
    
               item.aachecked=false;  
                });
  ...
  methods: {
    ...
    getMajorResourceList() {
      _this.$axios
        .get(_this.$api.majorResourceList, {
          params: {
        ...
        .then(res => {
          ...
          if (res.status == 200) {
            ...
            // if (data.length > 0) {
            //  _this.majorList =  _this.majorList.push(res.data.items);
            _this.majorList = _this.majorList.concat(res.data.items);
           ...
           // console.log("qqq _this.majorList", _this.majorList);
        ...
        els.forEach((item, index) => {
          ...
          if (document.body.clientWidth < 1522) {
            if (item.offsetHeight >= 63) {
              _this.majorList[index].isMore = true;
            } else {
              _this.majorList[index].isMore = false;
            }
          } else {
